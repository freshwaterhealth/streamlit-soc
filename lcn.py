...
st.title("FHI: Land Cover Naturalness")
gc.enable()
with st.expander("Input requirements", expanded=False):
    st.markdown("""
                **Provide input data in following format.** 
                    
                    1. For basin shapefile: Atleast .shp,.shx,.dbf and.prj as a zipped file.
...
st.sidebar.subheader("Step 3: Upload Raster Lookup File") 
...
if st.button('Calculate LCN Index'):
    lcn=0.
